...
public class AssimpSupport {
    public static Obj loadObj(String filename) {
        try (AIScene scene = Assimp.aiImportFile(AssimpSupport.class.getClassLoader().getResource(filename).getFile().substring(1), Assimp.aiProcess_Triangulate | Assimp.aiProcess_JoinIdenticalVertices | Assimp.aiProcess_FlipUVs)){
            if (scene == null) {
                return null;
            }
